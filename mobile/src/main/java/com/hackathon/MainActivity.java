...
public class MainActivity extends CardboardActivity implements CardboardView.StereoRenderer, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, DataApi.DataListener {
  ...
  private static final float YAW_LIMIT = 1.12f;
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    hudView = (CardboardHUDView) findViewById(R.id.hud);
  ...
  public void requestHead(){
    PutDataMapRequest req = PutDataMapRequest.create("/headReq");
    req.getDataMap().putInt("head", 1);
    ...
    void transEye(Eye eye){
        float halfInterpupillaryDistance = mCardboardView.getCardboardDeviceParams().getInterLensDistance() * 0.5F;
        if(eye.getType() == 1) halfInterpupillaryDistance *= -1;

        float eyeTranslate[] = new float[16];

        Matrix.setIdentityM(eyeTranslate, 0);
        Matrix.translateM(eyeTranslate, 0, halfInterpupillaryDistance, 0.0F, 0.0F);
        Matrix.multiplyMM(eye.getEyeView(), 0, eyeTranslate, 0, headView, 0);
    }
  /**
